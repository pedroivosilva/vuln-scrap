...
def zdi_csv(filename='ZDI - Tabela de Vulnerabilidades.csv'):
    ...
    driver.get(zdi + "/advisories/published/")
    zdi_advisories = driver.page_source
    soup = BeautifulSoup(zdi_advisories, 'html.parser')

    all_zdi_vulns = ()  # all ZDI vulnerabilities before checking its eligibility.
    zdi_vulns = []  # all ZDI vulnerabilities after checking its eligibility.
    ...
    for i in soup.find_all('tr', attrs={'id': 'publishedAdvisories'}):
        ...
        if (date_delta.days <= 7) and (td_cvss >= 7.0):
            ...
            #all_zdi_vulns = all_zdi_vulns + ([a_tag.text, zdi + a_tag['href'], td_release_date],)
